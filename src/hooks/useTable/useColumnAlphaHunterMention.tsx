...
const useColumnAlphaHunterMention = () => {
  const { accountExtendDetail } = useContext(AuthContext);
  const router = useRouter();

  const alphaHunterMention: Column<AlphaHunterMentionItem>[] = useMemo(
    () => [
      {
        Header: "Accounts",
        accessor: "profileImageUrl",
        Cell: ({ value, row }: CellProps<AlphaHunterMentionItem>) => {
          return (
            <>
              <Link href={`/alpha-hunter/${row.original.username}`}>
                <div className="flex items-center cursor-pointer">
                  ...
                  <div className="ml-2 ">
                    ...
                    {accountExtendDetail?.currentPlanKey ===
                    UserPayType.PREMIUM ? (
                      <p className="ml-1 mt-1 text-[16px]">
                        {row.original.name}
                      </p>
                    ) : (
      ...
      },
      {
        ...
        Cell: ({ value }: CellProps<AlphaHunterMentionItem>) => (
          <p className="pl-10">{moment(value).format("MM/DD/YYYY, HH:mm")}</p>
        ),
      },
      {
        Header: "Mentioned tweets",
        accessor: "attributes",
        Cell: ({ value }: CellProps<AlphaHunterMentionItem>) => {
...
export default useColumnAlphaHunterMention;
